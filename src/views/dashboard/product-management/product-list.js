...
const ProductList = () => {
  ...
  const DeleteProductModal = ({ product, handleCloseModal }) => {
    return (
      <Modal show={true} onHide={handleCloseModal}>
        ...
        <Modal.Body>
          <p>{`Bạn có chắc là muốn xóa sản phẩm (${product.name}) hay không?`}</p>
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="primary"
            onClick={() => {
              Product.deleteProduct(product.id).finally(() => {
                handleCloseModal();
                fetchList(page, perPage, searchText);
              });
            }}
      ...
      <Card.Header className="d-flex justify-content-between">
        ...
        {isStore ? (
          <Button className="btn-link text-center btn-primary btn-icon me-2 mt-lg-0 mt-md-0 mt-3">
            <Link to="/dashboard/product-management/product-list/product/new">
